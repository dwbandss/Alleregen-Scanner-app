...
import profileLogo from "../assets/Final Logo.png";
import { 
  FaUserEdit, FaTimes, FaQrcode, FaExclamationTriangle, 
...
export default function Home1({ user }) {
  ...
  const fetchScanData = () => {
    ...
    const allergens = stored.filter(s => s.allergens?.length > 0 && s.allergens[0] !== "None").length;
    const safeFoods = Math.round((stored.filter(s => s.safetyStatus === "Safe").length / (total || 1)) * 100);
  ...
  // ✅ Run on load and when "scan-updated" event fires
  useEffect(() => {
    fetchScanData();
  ...
  return (
    ...
    >
      ...
      <header className="home1-header glass-bg fade-in">
        <div className="header-left">
          <h1>Welcome, <span>{user?.name || user?.username || "User"}</span> 👋</h1>
          <p>Here's your allergen safety overview for today</p>
        </div>
        <img src={profileLogo} alt="Logo" className="profile-logo" />
      </header>
      ...
      {showPopup && (
        <div className="profile-popup-overlay">
          <div className="profile-popup glass-bg">
            ...
            <FaUserEdit className="popup-icon" />
            <h2>Set Up Your Profile</h2>
            <p>Complete your profile to personalize allergen detection and make your dashboard truly yours.</p>
            <button className="setup-btn" onClick={handleProfileSetup}>Go to My Profile</button>
          </div>
        </div>
      )}
    </div>
